...
import { ChevronLeft, ChevronRight, CheckCircle, Menu, BookOpen, MessageSquare, FileText, ListChecks } from "lucide-react";
...
export default function CourseLearnPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const modules = useMemo(
    () =>
      ((curriculumData?.curriculum || []) as Module[]).filter(
        (module) => ((module.lessons || []) as Lesson[]).length > 0
      ),
    [curriculumData]
  );
  ...
  useEffect(() => {
    ...
    const currentLessonStillVisible = modules.some((module) =>
      (module.lessons || []).some((lesson) => lesson._id === currentLesson._id)
    );

    if (!currentLessonStillVisible) {
  ...
  const courseLearnBase = `/courses/${course?.slug || course?._id || resolvedParams.slug}`;
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5f5f5]">
        <div className="text-center max-w-md px-4">
          <h1 className="text-2xl font-sans font-bold text-black"><T>Course content not available</T></h1>
          <p className="font-sans text-gray-600 mt-2">
            <T>This course does not have any published lessons with real content yet.</T>
          </p>
          <Button
            className="mt-4 rounded-[10px] bg-[#0052CC] hover:bg-[#003d99] text-white font-sans font-bold"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f5f5f5]">
        <div className="text-center max-w-md px-6">
          <h1 className="text-2xl font-sans font-bold text-black"><T>No lessons available</T></h1>
          <p className="font-sans text-gray-600 mt-2">
            <T>This course does not currently have any learner-visible lessons.</T>
          </p>
          <Button className="mt-4 rounded-[10px] bg-[#0052CC] hover:bg-[#003d99] text-white font-sans font-bold" onClick={() => router.push(`/courses/${course.slug || course._id}`)}>
            <T>Back to Course</T>
          </Button>
        </div>
      </div>
